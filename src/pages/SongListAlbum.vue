  ...
  <div>
    <div class="song-list-album">
      ...
      <div class="album-content">
        ...
        <el-tag>{{styleList[tempList.styleId].style}}</el-tag>
        <p><div class="el-icon-chat-dot-square" style="width: 30px;height: 30px"></div>{{commentNum}}</p>

        </div>
        <div class="album-score" style="margin-left: -50px">
          <div>
            <h3>歌单评分：</h3>
            <div>
...
<!--          <comment :play-id="songListId" :type="1"></comment>-->
          <song-comment :play-id="songListId" :type="1" style="margin-left: -350px" ref="comment" ></song-comment>
<!--        </div>-->
      </div>
    </div>
  </div>
...
export default {
  name: "SongListAblum",
  mixins: [mixins],
  components: {SongComment, Comment, AlbumContent },
  computed: {
    ...mapGetters(["listOfSongs", "tempList", "loginIn", "userId","styleList"]),
  },
  data() {
    return {
      songListId: "",
  ...
  created() {
    ...
    // console.log("style:"+this.styleList[this.tempList.styleId].style)
    console.log("styleList:"+JSON.stringify(this.styleList))
    // console.log("storeStyList:"+this.$store.state.styleList)
    // console.log(this.CommentNum)
  },
  mounted() {
    console.log("commentList:"+this.$refs.comment.commentList)
  },
  methods: {
    getRankScore() {
